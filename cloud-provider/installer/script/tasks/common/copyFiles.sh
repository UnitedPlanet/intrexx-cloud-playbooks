...
if      [ $OPERATING_SYSTEM == "linux" ]; then 
    ...
    ssh -o "StrictHostKeyChecking no" -i $SSH_KEY $AWS_ADMIN_USER_LINUX@$PROVISIONING_PUBLIC_IP "wget -nv -O ~/cloud-playbooks/files/$INTREXX_SETUP_LINUX https://download.unitedplanet.com/intrexx/90000/$INTREXX_SETUP_LINUX"
    ...
    ssh -o "StrictHostKeyChecking no" -i $SSH_KEY $AWS_ADMIN_USER_LINUX@$PROVISIONING_PUBLIC_IP "mv $TEMP_HOSTS_FILE ~/cloud-playbooks/hosts_$CLOUD_PROVIDER"
    echo "[PROVISIONING_MACHINE] - Download Intrexx setup"
    ssh -o "StrictHostKeyChecking no" -i $SSH_KEY $AWS_ADMIN_USER_LINUX@$PROVISIONING_PUBLIC_IP "wget -nv -O ~/cloud-playbooks/files/$INTREXX_SETUP_WIN https://download.unitedplanet.com/intrexx/90000/$INTREXX_SETUP_WIN" 
    echo "[PROVISIONING_MACHINE] - Download Intrexx setup finished"
fi
