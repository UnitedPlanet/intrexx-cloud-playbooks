...
if      [ $OPERATING_SYSTEM == "linux" ]; then 
    ...
    ssh -o "StrictHostKeyChecking no" -i $SSH_KEY $AWS_ADMIN_USER_LINUX@$PROVISIONING_PUBLIC_IP "wget -O ~/cloud-playbooks/cloud-playbooks/files/$INTREXX_SETUP_LINUX https://download.unitedplanet.com/intrexx/90000/$INTREXX_SETUP_LINUX"
    ...
    ssh -o "StrictHostKeyChecking no" -i $SSH_KEY $AWS_ADMIN_USER_LINUX@$PROVISIONING_PUBLIC_IP "mv $TEMP_HOSTS_FILE ~/cloud-playbooks/hosts_$CLOUD_PROVIDER"
    ssh -o "StrictHostKeyChecking no" -i $SSH_KEY $AWS_ADMIN_USER_LINUX@$PROVISIONING_PUBLIC_IP "wget -O ~/cloud-playbooks/files/$INTREXX_SETUP_WIN https://download.unitedplanet.com/intrexx/90000/$INTREXX_SETUP_WIN"
fi
