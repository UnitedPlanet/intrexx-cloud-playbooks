...
DB_DNS_ADDRESS=$(az sql server show --name $DB_SERVER_NAME --resource-group $RESOURCE_GROUP_NAME --query "fullyQualifiedDomainName" --output tsv)
...
echo "DB_DNS_ADDRESS=\$(az sql server show --name $DB_SERVER_NAME --resource-group $RESOURCE_GROUP_NAME --query \"fullyQualifiedDomainName\" --output tsv)"  >> $SCRIPT_BASE_PATH/$1
