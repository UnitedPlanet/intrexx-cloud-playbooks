...
elif    [ $AZ_DATABASE_TYPE == "postgres" ]; then    
  echo "[SQL_SERVER] - Creating postgresql server $DB_SERVER_NAME in location $AZ_AVAILABILITY_ZONE"
  az $AZ_DATABASE_TYPE server create \
    --name $DB_SERVER_NAME \
    --resource-group $RESOURCE_GROUP_NAME \
    --admin-user $DB_SERVER_ADMIN_USER \
...
az $AZ_DATABASE_TYPE server firewall-rule create \
...
DB_DNS_ADDRESS=$(az $AZ_DATABASE_TYPE server show --name $DB_SERVER_NAME --resource-group $RESOURCE_GROUP_NAME --query "fullyQualifiedDomainName" --output tsv)
...
echo "DB_DNS_ADDRESS=\$(az $AZ_DATABASE_TYPE server show --name $DB_SERVER_NAME --resource-group $RESOURCE_GROUP_NAME --query \"fullyQualifiedDomainName\" --output tsv)"  >> $SCRIPT_BASE_PATH/$1
