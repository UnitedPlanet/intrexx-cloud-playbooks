...
if [ "$GENERALIZED_STATE_COUNT" != '1' ]; then
    ...
    #For some strange reason if there is a default group, the command returns an error.
    az configure --defaults group=
    az vm delete \
        --resource-group $RESOURCE_GROUP_NAME \
        --name $APPSERVER_NAME \
        
    echo "[APPSERVER_VM] - Deleting resources linked to $APPSERVER_NAME under tag name $APPSERVER_RESOURCES_TAG"
    az resource delete \
